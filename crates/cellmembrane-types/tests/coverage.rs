...
use cellmembrane_types::composition::{CompositionSpec, MembraneComposition};
use cellmembrane_types::config::ShadowMode;
use cellmembrane_types::envelope::{BraidPolicy, EnvelopeLayer, EnvelopeTopology};
use cellmembrane_types::firewall::{FirewallProtocol, FirewallRule, FirewallRuleset};
...
#[test]
fn service_all_returns_eleven() {
    assert_eq!(MembraneService::all().len(), 11);
}
