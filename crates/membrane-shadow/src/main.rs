...
Temporal (waterFall temporal.*):
  ...
  temporal.cascade [--gate auto] [--source temporal] [--check] [--clone-missing] [--no-freshness]
                   [--with-harvest] [--with-rebuild] [--with-restart] [--check-installed]
                                   ...
                                   --with-restart: restart local NUCLEUS processes with new binaries
...
Plasmid (primal binary bootstrap):
  ...
  plasmid.harvest [--primal NAME] [--all] [--local] [--push] [--depot DIR] [--target TRIPLE] [--force] [--dry-run]
                                   Build from source, checksum, stage to depot (--push: auto-push to VPS)
  plasmid.push                     Push local depot binaries to VPS depot via SCP
...
Relay (K-Derm diderm relay chain):
  ...
  relay.ship [repo_path...]        Push to GitHub via outer membrane relay (ionic→weak outward)
