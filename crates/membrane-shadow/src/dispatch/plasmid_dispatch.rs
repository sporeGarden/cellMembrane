...
fn dispatch_composition(args: &[&str]) -> crate::Result<ShadowOutcome> {
    ...
    if let Some(gate) = gate_name {
        ...
        return match comp {
            Some(profile) => Ok(ShadowOutcome::ok_with(
                format!("{gate}: composition '{comp_name}' — {} primals, {} services",
                    profile.primals.len(), profile.services.len()),
                serde_json::json!({
                    "gate": gate,
            ...
            None => Ok(ShadowOutcome::ok(format!(
                "{gate}: composition '{comp_name}' (not defined in [compositions] section)"))),
        };
    }

    if let Some(name) = profile_name {
        return match manifest.composition(name) {
            Some(profile) => Ok(ShadowOutcome::ok_with(
            ...
            None => Ok(ShadowOutcome::fail(format!("composition '{name}' not found in manifest"))),
        };
    }
