...
//! Remote infrastructure dispatch — repo, mirror, service, token.
//!
//! SRP: Commands that operate on the VPS (golgiBody) via SSH and Forgejo API.
//! Contrast with `gate.rs` which handles local gate self-management.

use crate::cli::{self, TapMessage};
use crate::{ShadowConfig, ShadowOutcome, forgejo, service};
...
fn dispatch_service_template(binary: &str) -> ShadowOutcome {
    ...
    let security_socket = format!(
        "{socket_base}/{}",
        cellmembrane_types::service::constants::socket_filename(security_binary)
    );
