...
| **Escalation** | Phase 2 (NUCLEUS) — **stadial-ready** (Wave 107+, through Wave 155p) |
...
```bash
cargo test                  # 1277 tests — pedantic clippy clean
cargo clippy                # Zero warnings (pedantic + nursery + option_if_let_else)
cargo doc --open            # Full API documentation with doc-tests
```

Current state (Wave 155p): ~11k lines types, ~38k lines shadow. Crash-loop breaker
...
through Wave 155p are **DONE**. Full wave-by-wave audit trail is preserved in
...
| Code quality | 1277 tests, zero clippy warnings (pedantic), all files <800L | DONE |
