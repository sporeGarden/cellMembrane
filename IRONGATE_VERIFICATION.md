...
**Last validated:** Wave 155f (7-gate mesh live, 10-gate topology, subdomain routing, depot.primals.eco, 1219 tests, 0 clippy, J6 completed (gate.configure + gate.apply CLI), J8 foundation (key enrollment portal — step-ca SSH certificates, gate.keys CLI, enroll phase 8), J1+J2+J6 codified (ServiceSpec, plasmid.push, harvest --push, depot_sync Rust refactor), deep debt (Tower port const, bootstrap arch triple), cross-platform NUCLEUS (InitSystem dispatch), gate workload distribution, checksum format fix, blueGate/westGate topology, fleet convergence, BTSP, getrandom 0.4, 2026-07-28)
